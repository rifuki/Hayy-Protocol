...
export const AppStateProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Update wallet state when Sui wallet connection changes
  React.useEffect(() => {
    if (suiWallet.isConnected && suiWallet.address) {
      setWallet({ type: "sui", address: suiWallet.address });
    } else if (!suiWallet.isConnected && wallet?.type === "sui") {
      setWallet(undefined);
    }
  }, [suiWallet.isConnected, suiWallet.address, wallet?.type]);
  ...
  const connectSui = () => {
    // Sui wallet connection is handled by the WalletProvider
    console.log('Connect Sui wallet via wallet provider');
  };
